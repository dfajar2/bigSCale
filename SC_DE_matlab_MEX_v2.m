function [ D_scores ]=SC_DE_matlab_MEX_v2( total_data, N_pct , edges,indexes, name_file_xls,kg_or_ens,samples_names )
% SC_DE_matlab_MEX_v2 ************************************************************************
...
if length(indexes{1})>5000
    temp=indexes{1};
    temp=temp(randperm(numel(temp)));
	indexes{1}=temp(1:5000);
end
if length(indexes{2})>5000
    temp=indexes{2};
    temp=temp(randperm(numel(temp)));
	indexes{2}=temp(1:5000);
end
% ************************************************************************
...
if  length(name_file_xls)
    ...
    disp('Exporting ...... ')
    header={ 'GENE ID'	'GENE NAME'	'GENE TYPE' 'DESCRIPTION' 'D_score'	sprintf('Freq.%s',samples_names{1})	 sprintf('Exprs.%s',samples_names{1})	sprintf('Freq.%s',samples_names{2})	 sprintf('Exprs.%s',samples_names{2})	sprintf('Log2(%s / %s)',samples_names{1},samples_names{2}) }
    [ ~, ix]=sort(D_scores);
    to_write=[ kg_or_ens(:,1:4) mio_mat2cell([ D_scores f1' e1' f2' e2' log2(e2'./e1')])];    
