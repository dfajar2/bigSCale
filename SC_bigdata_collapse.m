...
% Internal script to convolute a large matrix upcon calculation of random cell distances

% scaling_factor k, as explained in the methods section
max_factor=5;
...
% progressively increasing percentiles, as explained in the methods section
MAX_DIST=prctile(reshape(distances,numel(distances),1),[5 10 20 30 40 50]);
disp('Calculated percentiles')


% ordering cells for distance
...
while 1
    ...
    while 1
        ...
        if any(available)
			...
			% Bocks is the output of the script. For every iCell stores the indexes of its original cells
            blocks=[ blocks; cells_in_use(ix) cells(ix,1:FACTOR) nan(1,decrease)];
            
            converted_cells=find(ismember(cells_in_use,blocks(end,:)));
            distances(converted_cells,:)=Inf; % setting to Inf the rows of the cells already used
            distances(ismember(cells(:,1:FACTOR),blocks(end,:)))=Inf; % setting to Inf the proper cells in the factor column
    ...
    paired_cells=reshape(blocks,numel(blocks),1);
    unpaired_cells=setdiff(all_cells,paired_cells);
    sprintf('Round %g, fixed %g cells, block size =%g,max_dist=%.1f (STEP_DIST=%g)',round,nnz(~isnan(paired_cells)),FACTOR+1,max_dist,STEP_DIST)

    converted_unpaired=find(ismember(cells_in_use,unpaired_cells));
    
	% Saving variable to hdd to free RAM, needed to process 1.3 million cells with 32 GB of RAM
    if round<5
